...
from .models import Task, Event  # ✅ Include Event here
from . import db
...
def task_list():
    ...
    tasks = Task.query.filter_by(user_id=user_id, due_date=selected_date).order_by(Task.created_at.desc()).all()
...
def schedule_daily():
    ...
    daily_items = sorted(
        tasks + events,
        key=lambda x: x.time if x.time else datetime.min.time()
    )
...
def schedule_weekly():
    ...
    for t in tasks:
        weekly_items[t.due_date].append(t)
    for e in events:
        weekly_items[e.date].append(e)
...
def schedule_monthly():
    ...
    events = Event.query.filter(
        Event.user_id == session["user_id"],
        Event.date >= start_month,
        Event.date <= end_month
    ).all()
    ...
    for t in tasks:
        monthly_items[t.due_date].append(t)
    for e in events:
        monthly_items[e.date].append(e)
